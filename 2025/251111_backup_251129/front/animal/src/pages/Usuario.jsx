...
const EyeIcon = ({ onClick, isVisible }) => (
  <span 
    onClick={onClick} 
    style={{ cursor: 'pointer', marginLeft: '5px', color: isVisible ? 'red' : 'green' }}
...
const Usuario = () => {
  ...
  const buscarUsuarios = async () => {
    try {
      ...
      const usuariosFormatados = dados.map(usuario => ({
        ...
        data_cadastro: usuario.data_cadastro ? new Date(usuario.data_cadastro).toLocaleString() : 'N/A',
        updatedAt: usuario.updatedAt ? new Date(usuario.updatedAt).toLocaleString() : 'N/A',
      }));
  ...
  const handleLogout = () => {
    // Lógica para destruir a sessão (destroir token, limpar localStorage, etc.)
        ...
        <Container fluid>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="me-auto">
              ...
              <Nav.Link as={Link} to="/historico_adocao">Histórico de Adoção</Nav.Link>
            </Nav>
      ...
      <Container className="my-5">
        ...
        <div className="table-responsive">
          {usuarios.length > 0 ? (
            ...
            <p className="alert alert-info">Nenhum usuário encontrado (Possível causa: erro na coexão com a base de dados.</p>
          )}
        </div>
